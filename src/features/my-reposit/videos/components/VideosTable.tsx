...
import { Check, X, Video, ArrowRight, Eye, Pencil, ChevronLeft, ChevronRight } from "lucide-react";
...
export default function VideosTable({
    videos, loading, currentPage, totalCount, pageSize, hasNext, hasPrevious,
    onPageChange, onPageSizeChange, onRefresh, onEdit,
}: VideosTableProps) {
    const [publishingVideo, setPublishingVideo] = useState<VideoType | null>(null);
    const [movingToReview, setMovingToReview] = useState<VideoType | null>(null);
    const [rejectingVideo, setRejectingVideo] = useState<VideoType | null>(null);
    const [rejectionReason, setRejectionReason] = useState("");
    const [isSubmitting, setIsSubmitting] = useState(false);
    const [viewingVideo, setViewingVideo] = useState<VideoType | null>(null);
    ...
    const handlePublish = async (video: VideoType) => {
        try { setIsSubmitting(true); await videoService.reviewVideo(video.id, { status: "published" }); toast.success(`"${video.title}" published successfully`); onRefresh(); }
        catch (error) { console.error("Failed to publish video:", error); toast.error("Failed to publish video"); }
        finally { setIsSubmitting(false); }
    };

    const handleMoveToReview = async (video: VideoType) => {
        try { setIsSubmitting(true); await videoService.moveVideoToReview(video.id); toast.success(`"${video.title}" moved to review`); onRefresh(); }
        catch (error) { console.error("Failed to move video to review:", error); toast.error("Failed to move video to review"); }
        finally { setIsSubmitting(false); }
    };

    const handleRejectSubmit = async () => {
        if (!rejectingVideo || !rejectionReason.trim()) return;
        try { setIsSubmitting(true); await videoService.reviewVideo(rejectingVideo.id, { status: "rejected", rejection_reason: rejectionReason.trim() }); toast.success(`"${rejectingVideo.title}" rejected`); setRejectingVideo(null); setRejectionReason(""); onRefresh(); }
        catch (error) { console.error("Failed to reject video:", error); toast.error("Failed to reject video"); }
        finally { setIsSubmitting(false); }
    };
        ...
        <>
            <div className="clay-card overflow-hidden" style={{ padding: 0 }}>
                {loading ? (
                    ...
                    <div className="p-8 text-center"><p className="text-gray-500 mb-4">No videos found. Try adjusting your filters.</p></div>
                ) : (
                    <>
                        <div className="overflow-x-auto min-w-0">
                            <table className="w-full table-auto divide-y divide-gray-100 min-w-max">
                                <thead style={{ background: "#f8f9fb", borderBottom: "1px solid rgba(0,0,0,0.06)" }}>
                                    <tr>
                                        <th className="px-4 py-3 text-left text-xs font-semibold text-gray-700 uppercase tracking-wider whitespace-nowrap w-1/4">Title</th>
                                        <th className="px-4 py-3 text-left text-xs font-semibold text-gray-700 uppercase tracking-wider whitespace-nowrap">Uploaded By</th>
                                        ...
                                        <th className="px-4 py-3 text-left text-xs font-semibold text-gray-700 uppercase tracking-wider whitespace-nowrap">Duration</th>
                                        ...
                                        <th className="px-4 py-3 text-left text-xs font-semibold text-gray-700 uppercase tracking-wider whitespace-nowrap">Views</th>
                                        ...
                                        <tr key={video.id} className="hover:bg-gray-50/60 transition-colors">
                                            <td className="px-4 py-4">
                                                <div className="flex items-start gap-3">
                                                    {video.thumbnail ? (
                                                        <img src={video.thumbnail} alt={video.title} className="w-9 h-9 rounded-lg object-cover shrink-0 mt-0.5" />
                                                    ) : (
                                                    ...
                                                    <div>
                                                        <div className="flex items-center gap-2">
                                                            <div className="text-sm font-medium text-gray-900 text-wrap">{video.title}</div>
                                                            {video.is_deleted && <span className="inline-flex items-center px-2 py-0.5 rounded text-xs font-medium bg-red-100 text-red-800 shrink-0">Deleted</span>}
                                                        </div>
                                                        <div className="text-xs text-gray-500 mt-0.5">{video.Institution || "—"}</div>
                                                    </div>
                                                </div>
                                            </td>
                                            <td className="px-4 py-4 text-sm text-gray-700">{video.uploaded_by}</td>
                                            <td className="px-4 py-4 text-sm text-gray-700">{video.speciality || "—"}</td>
                                            <td className="px-4 py-4 text-sm text-gray-700 whitespace-nowrap">{formatDuration(video.duration_seconds)}</td>
                                            <td className="px-4 py-4 whitespace-nowrap">{getStatusBadge(video.status)}</td>
                                            <td className="px-4 py-4 text-sm text-gray-700">{video.view_count}</td>
                                            <td className="px-4 py-4 whitespace-nowrap text-right">
                                                <div className="flex items-center justify-end gap-1">
                                                    <button onClick={() => setViewingVideo(video)} title="View Details" className="p-1.5 rounded-lg transition-all duration-200" style={{ color: "#6b96ff" }}
                                                    ...
                                                    <button onClick={() => onEdit(video)} title="Edit Video" className="p-1.5 rounded-lg transition-all duration-200" style={{ color: "#6b7280" }}
                                                    ...
                                                    {!video.is_deleted && video.status === "pending" && (
                                                        <button onClick={() => setMovingToReview(video)} disabled={isSubmitting} title="Move to Review" className="p-1.5 rounded-lg transition-all duration-200 disabled:opacity-50" style={{ color: "#ff9f47" }}
                                                    ...
                                                    {!video.is_deleted && (video.status === "pending" || video.status === "review") && (
                                                        <button onClick={() => setPublishingVideo(video)} disabled={isSubmitting} title="Publish" className="p-1.5 rounded-lg transition-all duration-200 disabled:opacity-50" style={{ color: "#4fcfa5" }}
                                                    ...
                                                    {!video.is_deleted && (video.status === "pending" || video.status === "review") && (
                                                        <button onClick={() => setRejectingVideo(video)} disabled={isSubmitting} title="Reject" className="p-1.5 rounded-lg transition-all duration-200 disabled:opacity-50" style={{ color: "#ff7070" }}
                        ...
                        {videos.length > 0 && (
                            <div className="px-5 py-3" style={{ background: "#eff1f5", boxShadow: "inset 2px 2px 5px rgba(0, 0, 0, 0.06), inset -2px -2px 5px rgba(255, 255, 255, 0.5)" }}>
                                <div className="flex flex-col sm:flex-row items-center justify-between gap-3">
                                    <div className="flex items-center gap-4">
                                        <div className="text-xs text-gray-600">Showing {(currentPage - 1) * pageSize + 1} to {Math.min(currentPage * pageSize, totalCount)} of {totalCount} videos</div>
                                        <div className="flex items-center gap-2">
                                            <label htmlFor="videoPageSize" className="text-xs text-gray-600">Per page:</label>
                                            <select id="videoPageSize" value={pageSize} onChange={(e) => onPageSizeChange(Number(e.target.value))}
            ...
            <ConfirmDialog isOpen={!!publishingVideo} onClose={() => setPublishingVideo(null)} onConfirm={() => { if (publishingVideo) handlePublish(publishingVideo); setPublishingVideo(null); }} title="Publish Video" message={`Are you sure you want to publish "${publishingVideo?.title}"?`} confirmText="Publish" cancelText="Cancel" variant="success" />
                    ...
                    <div className="flex min-h-full items-center justify-center p-4">
                        <div className="relative bg-white rounded-xl shadow-xl w-full max-w-md" onClick={(e) => e.stopPropagation()}>
                            <div className="p-6">
                                ...
                                <h3 className="text-lg font-semibold text-gray-900 mb-2">Reject Video</h3>
                                <p className="text-sm text-gray-600 mb-4">Are you sure you want to reject <span className="font-medium">"{rejectingVideo.title}"</span>? Please provide a reason.</p>
                                ...
                                <div className="flex items-center justify-end gap-3">
                                    <button onClick={() => { setRejectingVideo(null); setRejectionReason(""); }} className="clay-btn" style={{ fontSize: "13px", padding: "6px 16px" }}>Cancel</button>
            ...
            <VideoDetailsModal video={viewingVideo} isOpen={!!viewingVideo} onClose={() => setViewingVideo(null)} />
        </>
    );
}
