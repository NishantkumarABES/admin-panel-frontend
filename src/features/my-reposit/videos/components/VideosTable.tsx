...
export default function VideosTable({
    ...
    const handlePublish = async (video: VideoType) => {
        try {
            ...
            await videoService.reviewVideo(video.id, { status: "published" });
    ...
    const handleMoveToReview = async (video: VideoType) => {
        try {
            setIsSubmitting(true);
            await videoService.moveVideoToReview(video.id);
            toast.success(`"${video.title}" moved to review`);
            onRefresh();
        } catch (error) {
            console.error("Failed to move video to review:", error);
            toast.error("Failed to move video to review");
        } finally {
            setIsSubmitting(false);
        }
    };
    ...
    const handleRejectSubmit = async () => {
        ...
        try {
            setIsSubmitting(true);
            await videoService.reviewVideo(rejectingVideo.id, {
                status: "rejected",
                rejection_reason: rejectionReason.trim(),
            });
            toast.success(`"${rejectingVideo.title}" rejected`);
    ...
    return (
        <>
            <div className="bg-white rounded-lg border border-gray-200 overflow-hidden">
                {loading ? (
                    <div className="p-8 text-center text-gray-600">Loading videos...</div>
                ) : !videos?.length ? (
                                ...
                                <tbody className="divide-y divide-gray-200">
                                    {videos.map((video) => (
                                        <tr key={video.id} className="hover:bg-gray-50 transition-colors">
                                            ...
                                            <td className="px-6 py-4 text-gray-700">{video.uploaded_by}</td>
                                            <td className="px-6 py-4 text-gray-700">{video.speciality || "—"}</td>
                                            <td className="px-6 py-4 text-gray-700 whitespace-nowrap">{formatDuration(video.duration_seconds)}</td>
                                            <td className="px-6 py-4 whitespace-nowrap">{getStatusBadge(video.status)}</td>
                                            <td className="px-6 py-4 text-gray-700">{video.view_count}</td>
                                            <td className="px-6 py-4">
                            ...
                            <div className="flex flex-col sm:flex-row items-center justify-between gap-4">
                                <div className="flex items-center gap-4">
                                    <div className="text-sm text-gray-600">
                                        ...
                                        {Math.min(currentPage * pageSize, totalCount)} of {totalCount} videos
                                    </div>
                                    <div className="flex items-center gap-2">
                                        <label htmlFor="videoPageSize" className="text-sm text-gray-600">Per page:</label>
                                        <select
                                            id="videoPageSize"
                                            value={pageSize}
            ...
            <ConfirmDialog
                ...
                onConfirm={() => {
                    if (publishingVideo) handlePublish(publishingVideo);
                    setPublishingVideo(null);
                }}
                title="Publish Video"
                message={`Are you sure you want to publish "${publishingVideo?.title}"?`}
                confirmText="Publish"
                cancelText="Cancel"
                variant="success"
            />
            ...
            {rejectingVideo && (
                <div className="fixed inset-0 z-50 overflow-y-auto">
                    <div className="fixed inset-0 bg-black/50 transition-opacity" onClick={() => { setRejectingVideo(null); setRejectionReason(""); }} />
                        ...
                        <div className="relative bg-white rounded-lg shadow-xl w-full max-w-md" onClick={(e) => e.stopPropagation()}>
                            <div className="p-6">
                                ...
                                <p className="text-sm text-gray-600 mb-4">
                                    Are you sure you want to reject <span className="font-medium">"{rejectingVideo.title}"</span>? Please provide a reason.
                                </p>
                                <textarea
                                ...
                                <div className="flex items-center justify-end gap-3">
                                    <button
                                        onClick={() => { setRejectingVideo(null); setRejectionReason(""); }}
