...
import { Search, Filter, ChevronLeft, ChevronRight, Check, X, BookOpen } from "lucide-react";
import type { Book, BookType } from "./book.types";
...
export default function BooksView() {
  ...
  const fetchBooks = async () => {
    try {
      ...
      const response = await bookService.getPendingBooks(filters);
  ...
  }, [searchTerm, specialtyFilter, bookTypeFilter, currentPage, pageSize]);

  useEffect(() => {
    setCurrentPage(1);
  }, [searchTerm, specialtyFilter, bookTypeFilter]);
  ...
  const hasActiveFilters = searchTerm || specialtyFilter || bookTypeFilter;
  ...
  return (
    <div className="space-y-6 min-w-0 max-w-full">
      ...
      <div className="bg-white rounded-lg border border-gray-200 p-4 min-w-0">
        <div className="flex flex-col gap-4 lg:flex-row lg:items-center lg:gap-4 min-w-0">
          {/* Search */}
      ...
      <div className="bg-white rounded-lg border border-gray-200 overflow-hidden">
        {loading ? (
          ...
          <div className="p-8 text-center text-gray-600">
            No pending books found. Try adjusting your filters.
          </div>
        ) : (
          <>
