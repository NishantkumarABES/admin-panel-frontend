...
export default function ArticlesTable({
    articles, loading, currentPage, totalCount, pageSize, hasNext, hasPrevious,
    ...
    const handlePublish = async (article: Article) => {
        try { setIsSubmitting(true); await articleService.reviewArticle(article.id, { status: "published" }); toast.success(`"${article.title}" published successfully`); onRefresh(); }
        catch (error) { console.error("Failed to publish article:", error); toast.error("Failed to publish article"); }
        finally { setIsSubmitting(false); }
    };

    const handleMoveToReview = async (article: Article) => {
        try { setIsSubmitting(true); await articleService.moveArticleToReview(article.id); toast.success(`"${article.title}" moved to review`); onRefresh(); }
        catch (error) { console.error("Failed to move article to review:", error); toast.error("Failed to move article to review"); }
        finally { setIsSubmitting(false); }
    };

    const handleRejectSubmit = async () => {
        if (!rejectingArticle || !rejectionReason.trim()) return;
        try { setIsSubmitting(true); await articleService.reviewArticle(rejectingArticle.id, { status: "rejected", rejection_reason: rejectionReason.trim() }); toast.success(`"${rejectingArticle.title}" rejected`); setRejectingArticle(null); setRejectionReason(""); onRefresh(); }
        catch (error) { console.error("Failed to reject article:", error); toast.error("Failed to reject article"); }
        finally { setIsSubmitting(false); }
    };
        ...
        <>
            <div className="clay-card overflow-hidden" style={{ padding: 0 }}>
                {loading ? (
                    ...
                    <div className="p-8 text-center"><p className="text-gray-500 mb-4">No articles found. Try adjusting your filters.</p></div>
                ) : (
                    <>
                                ...
                                <tbody className="divide-y divide-gray-100">
                                    {articles.map((article) => (
                                        <tr key={article.id} className="hover:bg-gray-50/60 transition-colors">
                                            ...
                                            <td className="px-4 py-4 text-sm text-gray-700 break-words whitespace-normal" style={{ maxWidth: "200px" }}>{article.authors}</td>
                                            <td className="px-4 py-4 text-sm text-gray-700">{article.uploaded_by}</td>
                                            <td className="px-4 py-4 whitespace-nowrap"><span className="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-gray-100 text-gray-800 capitalize">{article.article_type?.replace(/_/g, " ")}</span></td>
                                            <td className="px-4 py-4 text-sm text-gray-700">{article.speciality || "—"}</td>
                                            <td className="px-4 py-4 whitespace-nowrap">{getStatusBadge(article.status)}</td>
                                            <td className="px-4 py-4 text-sm text-gray-700">{article.year}</td>
                                            <td className="px-4 py-4 whitespace-nowrap text-right">
                                                <div className="flex items-center justify-end gap-1">
                                                    <button onClick={() => setViewingArticle(article)} title="View Details" className="p-1.5 rounded-lg transition-all duration-200" style={{ color: "#6b96ff" }}
                                                    ...
                                                    <button onClick={() => onEdit(article)} title="Edit Article" className="p-1.5 rounded-lg transition-all duration-200" style={{ color: "#6b7280" }}
                        ...
                        {articles.length > 0 && (
                            <div className="px-5 py-3" style={{ background: "#eff1f5", boxShadow: "inset 2px 2px 5px rgba(0, 0, 0, 0.06), inset -2px -2px 5px rgba(255, 255, 255, 0.5)" }}>
                                <div className="flex flex-col sm:flex-row items-center justify-between gap-3">
                                    <div className="flex items-center gap-4">
                                        <div className="text-xs text-gray-600">Showing {(currentPage - 1) * pageSize + 1} to {Math.min(currentPage * pageSize, totalCount)} of {totalCount} articles</div>
                                        <div className="flex items-center gap-2">
                                            <label htmlFor="articlePageSize" className="text-xs text-gray-600">Per page:</label>
                                            <select id="articlePageSize" value={pageSize} onChange={(e) => onPageSizeChange(Number(e.target.value))}
            ...
            <ConfirmDialog isOpen={!!publishingArticle} onClose={() => setPublishingArticle(null)} onConfirm={() => { if (publishingArticle) handlePublish(publishingArticle); setPublishingArticle(null); }} title="Publish Article" message={`Are you sure you want to publish "${publishingArticle?.title}"?`} confirmText="Publish" cancelText="Cancel" variant="success" />
                    ...
                    <div className="flex min-h-full items-center justify-center p-4">
                        <div className="relative bg-white rounded-xl shadow-xl w-full max-w-md" onClick={(e) => e.stopPropagation()}>
                            <div className="p-6">
                                ...
                                <p className="text-sm text-gray-600 mb-4">Are you sure you want to reject <span className="font-medium">"{rejectingArticle.title}"</span>? Please provide a reason.</p>
                                ...
                                <div className="flex items-center justify-end gap-3">
                                    <button onClick={() => { setRejectingArticle(null); setRejectionReason(""); }} className="clay-btn" style={{ fontSize: "13px", padding: "6px 16px" }}>Cancel</button>
            ...
            <ArticleDetailsModal article={viewingArticle} isOpen={!!viewingArticle} onClose={() => setViewingArticle(null)} />
        </>
    );
}
